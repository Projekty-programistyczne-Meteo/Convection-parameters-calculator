// src/components/Converters/LiftingCondensationLevel.tsx
...
function LiftingCondensationLevel() {
  ...
  const [result, setResult] = useState<LiftingCondensationLevelResult | null>(
    null,
  );

  const handleCalculate = () => {
    const numericTemperature = Number(temperature.replace(',', '.'));
    const numericDewPoint = Number(dewPoint.replace(',', '.'));

    if (Number.isNaN(numericTemperature) || Number.isNaN(numericDewPoint)) {
      setResult(null);
      return;
    }

    const computed = calculateLiftingCondensationLevel({
      temperature: numericTemperature,
      dewPoint: numericDewPoint,
    });
          ...
          <p className="text-sm font-semibold text-stone-900 md:text-base">
            ...
            {result && (
              <span className="ml-2 text-sm font-bold text-green-700 md:text-base">
                = {formatValue(result.lcl)}
              </span>
            )}
          </p>

          <p className="mt-2 text-xs text-stone-600 md:text-sm">
            Enter air temperature and dew point temperature in °C. The result is
            the approximate lifting condensation level expressed in meters.
          </p>
        </div>
      </div>
    </section>
